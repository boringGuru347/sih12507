"""
Test Balanced Kolam Classifier
This script tests the balanced classifier on the full dataset to compare performance.
"""
...
class PrototypicalNetwork(nn.Module):
    """Balanced binary kolam classifier architecture"""
...
class TestKolamDataset(Dataset):
    """Dataset for testing balanced classification"""
...
def test_balanced_classifier():
    """Test the balanced binary classifier"""
    
    print("🚀 Testing Balanced Binary Kolam Classifier (Sikku vs Non-Sikku)")
    print("=" * 70)
    ...
    try:
        # Load full test dataset (all images)
        ...
        print("\n🤖 Loading trained balanced model...")
        ...
        print(f"✅ Balanced model loaded successfully from {model_path}")
        print(f"   Classes: {checkpoint.get('class_names', ['Non-Sikku', 'Sikku'])}")
        print(f"   Balance method: {checkpoint.get('balance_method', 'undersample')}")
        print(f"   Training config: {checkpoint.get('config', {})}")
        
        # Test the model
        print("\n🧪 Testing balanced model on full dataset...")
        ...
        balanced_accuracy = (non_sikku_accuracy + sikku_accuracy) / 2
        ...
        if incorrect_count > 0:
            ...
            for i, pred_info in enumerate(test_results['incorrect_predictions'][:10]):
        ...
        print(f"     Non-Sikku: {len(non_sikku_paths)} samples ({len(non_sikku_paths)/(len(non_sikku_paths)+len(sikku_paths))*100:.1f}%)")
        print(f"     Sikku: {len(sikku_paths)} samples ({len(sikku_paths)/(len(non_sikku_paths)+len(sikku_paths))*100:.1f}%)")
        print(f"   Balanced model performance on imbalanced data:")
        print(f"     Overall Accuracy: {test_results['accuracy']:.3f}")
        print(f"     Balanced Accuracy: {balanced_accuracy:.3f}")
        print(f"     Non-Sikku Recall: {test_results['per_class_recall'][0]:.3f}")
        print(f"     Sikku Recall: {test_results['per_class_recall'][1]:.3f}")
        ...
        plt.title('Confusion Matrix\n(Balanced Model on Full Dataset)')
        ...
        plt.subplot(1, 3, 3)
        sizes = [len(non_sikku_paths), len(sikku_paths)]
        labels = [f'Non-Sikku\n({len(non_sikku_paths)})', f'Sikku\n({len(sikku_paths)})']
        colors = ['lightcoral', 'lightblue']
        plt.pie(sizes, labels=labels, colors=colors, autopct='%1.1f%%', startangle=90)
        plt.title('Dataset Distribution')
        
        plt.tight_layout()
        plt.savefig('balanced_model_test_results.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"\n💾 Test results visualization saved to balanced_model_test_results.png")
        
        # Save detailed results
        with open('balanced_model_test_results.txt', 'w') as f:
            f.write("Balanced Binary Kolam Classifier Test Results\n")
            f.write("=" * 50 + "\n\n")
            f.write(f"Model: {model_path}\n")
            f.write(f"Balance method: {checkpoint.get('balance_method', 'undersample')}\n")
            f.write(f"Dataset: {len(test_paths)} images\n")
            f.write(f"Non-Sikku: {len(non_sikku_paths)} images ({len(non_sikku_paths)/(len(non_sikku_paths)+len(sikku_paths))*100:.1f}%)\n")
            f.write(f"Sikku: {len(sikku_paths)} images ({len(sikku_paths)/(len(non_sikku_paths)+len(sikku_paths))*100:.1f}%)\n\n")
            ...
            f.write("             Predicted\n")
            f.write("Actual    Non-Sikku  Sikku\n")
            f.write(f"Non-Sikku    {cm[0][0]:6d}  {cm[0][1]:5d}\n")
            f.write(f"Sikku        {cm[1][0]:6d}  {cm[1][1]:5d}\n")
        
        print(f"📄 Detailed results saved to balanced_model_test_results.txt")
